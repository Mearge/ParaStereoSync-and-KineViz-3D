...
ParaStereoSync is a Python toolkit for stereo-video motion measurement, combining camera calibration, synchronized template tracking, 3D triangulation, and 3D displacement visualization.
...
Keep these files with your experiment metadata if you need strict reproducibility.
...
1. Freeze release version in `README.md`, `docs1111/manuscript_for_Software_X.md`, and `CITATION.cff`.
2. Confirm `requirements.txt` resolves cleanly in a new environment.
3. Verify all command examples in docs run with current script names.
4. Archive a minimal reproducible dataset or provide access instructions.
5. Include representative output artifacts (tracking CSV, 3D CSV, figures).
6. Add final author names, affiliations, and ORCID IDs in `CITATION.cff`.
7. Update repository URL and DOI fields in manuscript and citation metadata.
8. Confirm license file and software metadata are consistent.
9. Validate manuscript sections against released software behavior.
10. Tag the release and record commit hash used in the publication.

## License and Citation

- License file: `LICENSE`
- Citation metadata is provided in `CITATION.cff` and must be finalized with author and DOI details before publication.
